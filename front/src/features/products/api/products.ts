...
import type { editProducts, productsCreate } from "../schemas/SchemaProducts"
...
export const SendProductsEdit = async (products: editProducts): Promise<ResponseServer> => {
    const formData = new FormData()
    ...
    try {
        const ReponseProducts = await fetch('http://localhost:3000/products/edit', {
            method: 'PATCH',
            credentials: 'include',
            body: formData
        })
